...
const Sidebar = (props: Props) => {
  ...
  return <Box sx={{
    ...
    <Stack textAlign="center" spacing={2}>
      <Box mt={4}>
        <Typography variant="h6">{currentTheme === AppTheme.LIGHT ? "Light Mode" : "Dark Mode"}</Typography>
        <Switch sx={{ margin: 'auto' }} checked={currentTheme === AppTheme.LIGHT} onChange={(_, checked) => {
